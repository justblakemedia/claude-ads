# Blinq to Zapier to OneDrive to Obsidian Setup
...
Try first: in Zapier, search "Blinq" and create a Zap with trigger **"New Contact"**. Connect your Blinq account.
...
- File Name: `{{name}} ({{company}}).md`
  Parens prevent filename collisions when two people share a name and avoid OneDrive-unsafe characters.
- File Content:
...
3. After OneDrive sync (30 to 60 sec), the file is visible in Obsidian and on disk locally.

## Gotchas

- **YAML phone quoting**: always wrap `{{phone}}` in double quotes. Numbers with leading `+` break YAML otherwise.
- **OneDrive sync delay**: new contacts take 30 to 60 seconds to appear locally. Not instant.
...
- **Duplicate contacts**: if someone re-shares their card, Zapier will create a second file. Either accept dupes or add a "Find File" step before Create to skip existing.
...
Then ask Claude things like "what's Jane Doe's email?" or "who did I meet at SaaStr?" and it will grep the `People/` folder on demand.
