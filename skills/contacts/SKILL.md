---
name: contacts
description: "Look up people in the user's Obsidian vault (JB vault). Contacts auto-sync from Blinq business card scans via Zapier → OneDrive. Use when user asks about a person by name, company, email, or when they met someone, or wants to pull contact info."
user-invokable: true
---

# Contacts (Blinq → Obsidian)

## Source of truth

Contacts live in the user's Obsidian vault, one markdown file per person, in the `People/` subfolder. Files are written automatically by a Zapier automation: Blinq business card scan → Zapier → OneDrive → Obsidian vault.
...
Filename format: `{name} — {company}.md` (em dash separator, prevents collisions).
...
`/contact <query>` → search and display matches.

## Setup

The Zapier pipeline must be configured once before anything lands in the vault. See `assets/zapier-setup.md` for the full Blinq → Zapier → OneDrive → Obsidian walkthrough.
